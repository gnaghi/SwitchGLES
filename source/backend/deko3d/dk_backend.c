...
const sgl_backend_ops_t dk_backend_ops = {
    ...
    .blit_framebuffer = dk_blit_framebuffer,
...
int dk_init(sgl_backend_t *be, void *device) {
    ...
    for (int i = 0; i < SGL_FB_NUM; i++) {
        ...
        cmdMaker.userData = dk;
        cmdMaker.cbAddMem = dk_cmdbuf_overflow_cb;
    ...
    /* Initialize ALL descriptor memory to zero.
     * Both image descriptors (first half) and sampler descriptors (second half)
     * must be zeroed so unused slots don't contain garbage that causes GPU faults
     * when TIC/TSC caches are invalidated.
     *
     * IMPORTANT: Do NOT fill sampler slots with valid (non-zero) DkSamplerDescriptors!
     * Non-zero sampler descriptors combined with DkInvalidateFlags_Descriptors in
     * frame-start barriers causes a deko3d STARTUP CRASH. All-zero descriptors
     * are safe because the hardware treats them as null/disabled. */
    memset(dkMemBlockGetCpuAddr(dk->descriptor_memblock), 0, SGL_DESCRIPTOR_MEM_SIZE);
    ...
    memset(dk->texture_level_mask, 0, sizeof(dk->texture_level_mask));
    ...
    /* Create 1x1 black fallback texture at slot 0 for incomplete texture sampling.
     * Per GLES2 §3.7.10, incomplete textures return (0,0,0,1). */
    dk_create_black_texture(dk);
...
void dk_shutdown(sgl_backend_t *be) {
    ...
    SGL_TRACE_BACKEND("Backend shutdown complete");
}
