...
DkImageFormat dk_convert_format(GLenum internalformat, GLenum format, GLenum type) {
    ...
    /* Packed pixel formats (16-bit): convert to RGBA8 during staging upload.
     * Native packed DkImageFormats have ambiguous component ordering on Maxwell,
     * so we unpack to RGBA8 in the staging copy path for guaranteed correctness. */
    if (format == GL_RGBA && type == GL_UNSIGNED_BYTE) {
        return DkImageFormat_RGBA8_Unorm;
    }
    if (format == GL_BGRA_EXT && type == GL_UNSIGNED_BYTE) {
        /* Use RGBA8 internally — BGRA8_Unorm crashes as render target on Tegra X1.
         * BGRA byte order is handled by CPU-side swizzle during staging upload
         * and ReadPixels (same approach as renderbuffers). */
        return DkImageFormat_RGBA8_Unorm;
    }
...
void dk_get_attrib_format(GLenum type, GLint size, GLboolean normalized,
    ...
    /* Type: GLES2 vertex attributes are always float in the shader.
     * normalized=TRUE  → Snorm/Unorm (int→float in [-1,1] or [0,1])
