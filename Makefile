...
CFLAGS	:=	-g -Wall -Wextra -Wunused-function -Wunused-variable -O0 \
			-ffunction-sections \
			-fdata-sections \
			$(ARCH)
...
#---------------------------------------------------------------------------------
# Bundled static libraries (merged into libSwitchGLES.a)
#   Users only need: -lSwitchGLES -lnx -lstdc++ -lm
#---------------------------------------------------------------------------------
LIBUAM_A	:= $(CURDIR)/../uam/builddir/libuam.a
LIBDEKO3D_A	:= $(DEVKITPRO)/libnx/lib/libdeko3d.a
...
lib/libSwitchGLES.a: $(SOURCES) $(INCLUDES) | lib_dir build
	...
	-f $(CURDIR)/Makefile
	@echo "Bundling deko3d and libuam into libSwitchGLES.a..."
	@mkdir -p build/_deko3d build/_uam
	@cd build/_deko3d && $(AR) x $(LIBDEKO3D_A)
	@cd build/_uam && $(AR) x $(LIBUAM_A)
	@cd build/_deko3d && for f in $$(find . -name '*.o' -printf '%f\n'); do mv "$$f" "__dk3d_$$f"; done
	@cd build/_uam && for f in $$(find . -name '*.o' -printf '%f\n'); do mv "$$f" "__uam_$$f"; done
	@$(AR) rcs $@ $$(find build/_deko3d build/_uam -name '*.o')
	@rm -rf build/_deko3d build/_uam
	@echo "libSwitchGLES.a ready (includes deko3d + libuam)"
...
install: library
	...
	@mkdir -p $(DEVKITPRO)/portlibs/switch/include/switchgles/GLES3
	...
	@cp include/GLES3/*.h $(DEVKITPRO)/portlibs/switch/include/switchgles/GLES3/
	...
	@echo "  Headers: $(DEVKITPRO)/portlibs/switch/include/switchgles/{EGL,GLES2,GLES3,KHR}/"
